...
import 'package:flutter_mimc_ui/utils/permission.dart';
import 'package:flutter_mimc_ui/view/emoji/emoji_picker.dart';
import 'package:flutter_mimc_ui/view/message.dart';
import 'package:flutter_mimc_ui/view/voice/voice.dart';
  ...
  Widget _buildMessageRow(ImMessage message, int index) {
    ...
    return Column(
      children: <Widget>[
        ...
        ImMessageItemView(
          message: message,
          avatarUrl: widget.currentUser.uid == message.fromAccount
              ? widget.currentUser.avatarUrl
              : widget.toUser.avatarUrl,
          messageAlign: widget.currentUser.uid == message.fromAccount
              ? MessageRightAlign
              : MessageLeftAlign,
        )
      ],
  ...
  */
  void _submitMsg(String text) async {
    ...
    ImMessage message = ImMessage(
        ...
        fromAccount: widget.currentUser.uid,
        toAccount: widget.toUser.uid,
        secondaryPayload:
            ImMessagePayload(text: text, bizType: ImMessageType.text));
    ...
    MIMC.sendText(widget.currentUser.uid, widget.toUser.uid, text);
    //发送到服务器
  }
